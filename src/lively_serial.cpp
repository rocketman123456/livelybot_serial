...
void lively_serial::recv()
{
    ...
    while (ros::ok() && init_flag)
    {
        ...
        if (*(uint8_t*)&_result[0] == 0xFD && *(uint8_t*)&_result[1] == 0xFE)
        {
            ...
            {
                // ROS_INFO("%X %X",cdc_acm_tx_message.crc16,crc_ccitt(0x0000, (const uint8_t *)&cdc_acm_tx_message,
                // sizeof(cdc_acm_tx_message_t) - 2));
      ...
      // this method is read the avaliable from the buffer
        ...
        if (_result.length() > 0)
        {
            if (*(uint8_t*)&_result[0] == 0xFD && *(uint8_t*)&_result[1] == 0xFE)
            {
                ...
                // printf("0x%04X\n ", *(uint16_t *)&_result[15]);
                // printf("0x%04X\n ", crc_ccitt(0x0000, (const uint8_t *)&_result[0], sizeof(cdc_acm_tx_message_t) -
                // 2));

                if (*(uint16_t*)&_result[15] ==
